...
/*			iget=sscanf(str+3,"%d %f %f %d %d %d %f %f %f %f %f %f",&param1,&param2,&param3,&param4,&param5,&param6,
			...
			do {						/*loop over lines in file until found // beginning the parameter line*/
				if(fgets(str,SMAX-2,fp));
					...
					sprintf(smess,"error in reading dataset %ld : cannot read datasetfile prout ",dataset);
					write(ERRORFILE,smess);
					exit(1);
				}
				swhiteline[0]='\0';
...
/* reads inputfile for values of parameters and check them against legal range*/
	...
	if((fp=fopen(inputfile,"r"))!=NULL)
	{
		...
		if((lget>NMAX) || (lget<1)){                            /*test N*/
			sprintf(smess,"error in reading initial conditions : N=%ld is higher than Nmax=%d or invalid",lget,NMAX); 
			write(ERRORFILE,smess);
			exit(1); 
		} 	else *N=lget;
		...
		if((lget>NALL_LIFE_MAX) || (lget<1)){                            /*test nall_life*/
			sprintf(smess,"error in reading initial conditions : nall_life=%ld is higher than max value=%d or invalid",lget,NALL_LIFE_MAX); 
			write(ERRORFILE,smess);
			exit(1); 
		} 	else *nall_life=lget;
